// https://github.com/denoland/deno/blob/v2.2.8/runtime/fmt_errors.rs
...
impl deno_core::error::ErrorFormat for AnsiColors {
    fn fmt_element(element: deno_core::error::ErrorElement, s: &str) -> std::borrow::Cow<'_, str> {
        use deno_core::error::ErrorElement::*;
        match element {
            Anonymous | NativeFrame | FileName | EvalOrigin => colors::cyan(s).to_string().into(),
...
fn format_js_error_inner(
    ...
    if let Some(circular) = &circular {
        if js_error.is_same_error(circular.reference.to) {
            write!(s, " {}", colors::cyan(format!("<ref *{}>", circular.index))).unwrap();
        }
    }
    ...
    if let Some(cause) = &js_error.cause {
        ...
        let error_string = if is_caused_by_circular {
            colors::cyan(format!("[Circular *{}]", circular.unwrap().index)).to_string()
        } else {
    ...
    if !suggestions.is_empty() {
        ...
        for (index, suggestion) in suggestions.iter().enumerate() {
            ...
            match suggestion.kind {
                FixSuggestionKind::Hint => write!(s, "{} ", colors::cyan("hint:")).unwrap(),
                FixSuggestionKind::Info => write!(s, "{} ", colors::yellow("info:")).unwrap(),
                FixSuggestionKind::Docs => write!(s, "{} ", colors::green("docs:")).unwrap(),
            };
...
fn get_suggestions_for_terminal_errors(e: &JsError) -> Vec<FixSuggestion> {
    if let Some(msg) = &e.message {
        ...
        {
            if let Some(file_name) = e.frames.first().and_then(|f| f.file_name.as_ref()) {
        ...
        FixSuggestion::docs("https://docs.deno.com/go/commonjs"),
            ...
            return vec![
                ...
                FixSuggestion::hint_multiline(&[
                    cstr!(
                        "Import it explicitly with <u>import { Buffer } from \"node:buffer\";</>,"
                    ),
                    cstr!(
                        "or run again with <u>--unstable-node-globals</> flag to add this global."
                    ),
                ]),
            ];
        ...
        FixSuggestion::hint_multiline(&[
          ...
          cstr!("or run again with <u>--unstable-node-globals</> flag to add this global."),
            ...
            return vec![
                ...
                FixSuggestion::hint("Run again with `--unstable-kv` flag to enable this API."),
            ];
        } else if msg.contains("cron is not a function") {
            return vec![
                FixSuggestion::info("Deno.cron() is an unstable API."),
                FixSuggestion::hint("Run again with `--unstable-cron` flag to enable this API."),
            ];
        } else if msg.contains("WebSocketStream is not defined") {
            return vec![
                FixSuggestion::info("new WebSocketStream() is an unstable API."),
                FixSuggestion::hint("Run again with `--unstable-net` flag to enable this API."),
            ];
            ...
            return vec![
                ...
                FixSuggestion::hint("Run again with `--unstable-webgpu` flag to enable this API."),
            ];
